...
const UserProfile = ({ setConfirmationPostMessage }: UserProfileProps) => {
  // const { id } = useParams();
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      setConfirmationPostMessage(`Votre photo a bien été enregistée`);
    ...
    <>
      <div className="grid grid-cols-5 grid-rows-5 gap-8 md:w-1/2 mt-8 md:m-auto md:mt-8 font-nunito">
        <div className="col-span-5 lg:col-span-1">
          ...
          ></input>
          {uploadIsDisplayed && (
            <button className="border p-2 rounded-md w-28" onClick={onSubmit}>
              {isSubmitting ? (
                <div role="status ">
                  <svg
                    aria-hidden="true"
